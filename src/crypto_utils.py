...
def sign_certificate(cert_bytes: bytes, sk: bytes) -> tuple[bytes, bool, str]:
    ...
    try:
        ...
        # Preserve existing TEXT metadata only (skip binary chunks like ICC profiles)
        for key, value in img.info.items():
            # Skip our signature fields and binary data
            if key in ['signature', 'imageHash', 'signer', 'description', 'icc_profile']:
                continue
            # Only preserve string metadata
            if isinstance(value, str):
                try:
                    metadata.add_text(key, value)
                except:
                    pass  # Skip if it can't be added
        ...
        output = BytesIO()
        # Preserve ICC profile separately if it exists
        save_kwargs = {'format': 'PNG', 'pnginfo': metadata}
